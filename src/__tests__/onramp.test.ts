...
describe('estimateOnrampOutput', () => {
  describe('edge cases', () => {
    it('returns zeros for amount of 0', () => {
      expect(estimateOnrampOutput(0, PROVIDER_MOONPAY)).toEqual({
        fee: 0,
        receive: 0,
      });
      expect(estimateOnrampOutput(0, PROVIDER_TRANSAK)).toEqual({
        fee: 0,
        receive: 0,
      });
    });
    ...
    it('returns zeros for negative amounts', () => {
      expect(estimateOnrampOutput(-100, PROVIDER_MOONPAY)).toEqual({
        fee: 0,
        receive: 0,
      });
      expect(estimateOnrampOutput(-1, PROVIDER_TRANSAK)).toEqual({
        fee: 0,
        receive: 0,
      });
    });
