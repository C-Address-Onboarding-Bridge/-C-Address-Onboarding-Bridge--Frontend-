...
export function FeeSelector({ network, onFeeChange }: FeeSelectorProps) {
  ...
  const handleUnitChange = (unit: 'stroops' | 'xlm') => {
    ...
    if (tiers && customInput) {
      const stroops =
        unit === 'stroops' ? customInput : xlmToStroops(customInput);
      onFeeChange(stroops, validateFee(stroops, tiers.baseFee));
    }
  };
  ...
  return (
    <div className="space-y-3">
      {tiers?.congested && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2">
          ...
          <span className="text-xs text-amber-300">
            Network congested — consider using Fast fee
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-2">
        {(
          ['slow', 'standard', 'fast'] as Array<'slow' | 'standard' | 'fast'>
        ).map((tier) => (
          <button
            key={tier}
            onClick={() => handleSelectTier(tier)}
            disabled={loading}
            className={`flex flex-col items-center rounded-lg border px-2 py-3 text-xs font-medium transition-colors disabled:opacity-40 ${
              selected === tier
          ...
          >
            ...
            {loading ? (
              ...
              <span className="mt-1 font-mono text-[var(--text-muted)]">
        ...
        <button
          ...
          className={`flex flex-col items-center rounded-lg border px-2 py-3 text-xs font-medium transition-colors ${
            selected === 'custom'
              ? 'border-[var(--primary)] bg-[var(--primary)]/10 text-[var(--primary-light)]'
              : 'border-[var(--border)] hover:bg-[var(--surface-2)]'
          }`}
      ...
      {selected === 'custom' && (
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2 text-sm transition-colors focus:border-[var(--primary)] focus:outline-none"
          />
          <select
            value={customUnit}
            onChange={(e) =>
              handleUnitChange(e.target.value as 'stroops' | 'xlm')
            }
            className="rounded-lg border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2 text-sm transition-colors focus:border-[var(--primary)] focus:outline-none"
          >
        ...
        <p className="text-xs text-[var(--text-muted)]">
          ...
          {!validateFee(customStroops, tiers.baseFee) && (
            <span className="ml-2 text-[var(--error)]">
              Below minimum ({tiers.baseFee} stroops)
            </span>
          )}
        </p>
      )}
