...
export async function simulateTransaction(
  ...
  try {
    ...
    if (responseAsRecord.error) {
      const errorMessage =
        (responseAsRecord.error as Record<string, unknown>).message ||
        'Simulation failed';
    ...
    result.warnings.push(
      'Could not simulate transaction. Please review carefully before signing.'
    );
    return result;
  }
}
...
export function validateSimulationAgainstForm(
  ...
  if (!simulationResult.isSuccessful && simulationResult.errors.length > 0) {
    warnings.push(
      `⚠️ Transaction simulation failed: ${simulationResult.errors[0]}`
    );
  }
  ...
  const hasPaymentEffect = simulationResult.effects.some(
    (e) => e.type === 'payment'
  );
  if (!hasPaymentEffect && simulationResult.isSuccessful) {
    warnings.push(
      'No payment effect found in simulation. Please verify the transaction details.'
    );
  }
