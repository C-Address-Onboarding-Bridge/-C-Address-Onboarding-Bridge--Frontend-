...
export function validateEnvironment(): string[] {
  ...
  if (!process.env[ENV_BRIDGE_CONTRACT_ID]) {
    warnings.push(
      `${ENV_BRIDGE_CONTRACT_ID} is not set; bridge will fall back to direct payment`
    );
  }
  if (!process.env[ENV_MOONPAY_API_KEY]) {
    warnings.push(
      `${ENV_MOONPAY_API_KEY} is not set; Moonpay onramp will be unavailable`
    );
  }
  if (!process.env[ENV_TRANSAK_API_KEY]) {
    warnings.push(
      `${ENV_TRANSAK_API_KEY} is not set; Transak onramp will be unavailable`
    );
  }
  return warnings;
}
...
export async function getAccountBalances(
  ...
  try {
    ...
    const balances = (account.balances as HorizonBalance[]).map((b) => ({
      asset:
        b.asset_type === NATIVE_ASSET_TYPE
          ? ASSET_XLM
          : b.asset_code || UNKNOWN_ASSET,
      amount: b.balance,
    }));
    const total =
      balances.find((b) => b.asset === ASSET_XLM)?.amount || BALANCE_INITIAL;
...
export async function fetchRecentTransactions(
  ...
  try {
    ...
    return (payments.records as HorizonPayment[]).map((p) => ({
      ...
      amount: p.amount || '0',
      asset:
        p.asset_type === NATIVE_ASSET_TYPE
          ? ASSET_XLM
          : p.asset_code || ASSET_XLM,
...
export async function buildAndSubmitPayment(
  ...
  const tx = new TransactionBuilder(account, {
    ...
    .addOperation(
      Operation.payment({ destination: destinationAddress, asset, amount })
    )
...
export async function buildBridgeTransaction(
  ...
  return new TransactionBuilder(account, {
    ...
    .addOperation(
      Operation.payment({
        destination: BRIDGE_CONTRACT_ID || cAddress,
        asset: Asset.native(),
        amount,
      })
    )
...
export async function bridgeViaContract(
  ...
  try {
    ...
    throw new Error(
      `Failed to load account ${sourceAddress}: ${e instanceof Error ? e.message : String(e)}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Polling failed for tx ${sendResult.hash}: ${e instanceof Error ? e.message : String(e)}`
    );
  }

  if (polled.status !== 'SUCCESS') {
    throw new Error(
      `Bridge contract call did not succeed (status: ${polled.status})`
    );
  }
...
async function simulateContractRead(
  ...
  return (
    (sim as rpc.Api.SimulateTransactionSuccessResponse).result?.retval ?? null
  );
}
...
function addressToScVal(address: string) {
  if (StrKey.isValidEd25519PublicKey(address)) {
    return nativeToScVal(
      Address.account(StrKey.decodeEd25519PublicKey(address)),
      { type: 'address' }
    );
  }
  return nativeToScVal(Address.contract(StrKey.decodeContract(address)), {
    type: 'address',
  });
}
...
export async function getTokenAllowance(
  ...
  const tx = new TransactionBuilder(account, {
    ...
    .addOperation(
      contract.call('allowance', addressToScVal(owner), addressToScVal(spender))
    )
    .setTimeout(30)
    .build();
...
export async function approveToken(
  ...
  if (sendResult.status === 'ERROR') {
    throw new Error(
      `Approve transaction failed: ${JSON.stringify(sendResult.errorResult)}`
    );
  }
  const polled = await server.pollTransaction(sendResult.hash, {
    attempts: 20,
    sleepStrategy: rpc.BasicSleepStrategy,
  });
  if (polled.status !== 'SUCCESS') {
...
export async function deployContract(
  ...
  const signedUploadXDR = await defaultSigner(
    preparedUpload.toXDR(),
    passphrase
  );
  const uploadResult = await server.sendTransaction(
    TransactionBuilder.fromXDR(signedUploadXDR, passphrase)
  );
  if (uploadResult.status === 'ERROR')
    throw new Error(
      `WASM upload failed: ${JSON.stringify(uploadResult.errorResult)}`
    );
  const uploadPolled = await server.pollTransaction(uploadResult.hash, {
    attempts: 30,
    sleepStrategy: rpc.BasicSleepStrategy,
  });
  if (uploadPolled.status !== 'SUCCESS')
    throw new Error(`WASM upload did not succeed: ${uploadPolled.status}`);

  const wasmHash = scValToNative(
    (uploadPolled as rpc.Api.GetSuccessfulTransactionResponse).returnValue!
  ) as string;
  ...
  const signedDeployXDR = await defaultSigner(
    preparedDeploy.toXDR(),
    passphrase
  );
  const deployResult = await server.sendTransaction(
    TransactionBuilder.fromXDR(signedDeployXDR, passphrase)
  );
  if (deployResult.status === 'ERROR')
    throw new Error(
      `Contract deploy failed: ${JSON.stringify(deployResult.errorResult)}`
    );
  const deployPolled = await server.pollTransaction(deployResult.hash, {
    attempts: 30,
    sleepStrategy: rpc.BasicSleepStrategy,
  });
  if (deployPolled.status !== 'SUCCESS')
    throw new Error(`Contract deploy did not succeed: ${deployPolled.status}`);

  const contractId = scValToNative(
    (deployPolled as rpc.Api.GetSuccessfulTransactionResponse).returnValue!
  ) as string;
...
export async function upgradeContract(
  ...
  const tx = new TransactionBuilder(account, {
    ...
    .addOperation(
      contract.call(
        '__upgrade',
        nativeToScVal(Buffer.from(newWasmHash, 'hex'), { type: 'bytes' })
      )
    )
    .setTimeout(STELLAR_TX_TIMEOUT_SECONDS)
    .build();
  ...
  if (result.status === 'ERROR')
    throw new Error(`Upgrade failed: ${JSON.stringify(result.errorResult)}`);
  const polled = await server.pollTransaction(result.hash, {
    attempts: 30,
    sleepStrategy: rpc.BasicSleepStrategy,
  });
  if (polled.status !== 'SUCCESS')
    throw new Error(`Upgrade did not succeed: ${polled.status}`);
  return result.hash;
}
...
export async function getContractState(
  ...
  try {
    ...
    const wasmHash = entry
      ? Buffer.from(
          (entry.val as { contractData?: { val?: { wasm_hash?: Uint8Array } } })
            ?.contractData?.val?.wasm_hash ?? []
        ).toString('hex')
      : null;
