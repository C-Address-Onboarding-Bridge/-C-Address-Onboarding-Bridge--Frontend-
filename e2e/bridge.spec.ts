...
test('bridge form shows validation error for invalid C-address', async ({
  page,
}) => {
  await page.goto('/bridge');
  ...
  await inputs
    .nth(0)
    .fill('GABC123456789012345678901234567890123456789012345678901234');
...
test('bridge form proceed button disabled when empty', async ({ page }) => {
  await page.goto('/bridge');
  await expect(
    page.locator('button:has-text("Review Bridge Transaction")')
  ).toBeDisabled();
});
...
test('bridge form advances to review when valid', async ({ page }) => {
  ...
  await inputs
    .nth(0)
    .fill('GAAZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN');
