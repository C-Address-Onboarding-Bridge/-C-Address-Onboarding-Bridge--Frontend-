...
const config: StorybookConfig = {
  ...
  webpackFinal: async (config) => {
    config.resolve.alias = {
      ...
      '@/components/wallet-provider': path.resolve(
        __dirname,
        '../src/__mocks__/wallet-provider.tsx'
      ),
      '@/hooks/use-keyboard-shortcuts': path.resolve(
        __dirname,
        '../src/__mocks__/hooks/use-keyboard-shortcuts.ts'
      ),
    };
